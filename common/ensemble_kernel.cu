...
//offset is 2 component vector {shifting starting index, shifting direction}
...
__device__ __forceinline__ float4 read_strent(int i, int j, int offset, float4 new_strent) {
    return fetch_new_strent(j, offset) ? new_strent : tex2D(t_a_QN, make_offset(j, offset), i);
}
...
//    STRENT_KERNEL_LINEAR
//Calculates probabilities of entanglement shuffle, creation and destruction by CD on all strands
//Memory and kernel structure: 2D kernel; i- entanglement index j - chain index
template<int type> __global__ __launch_bounds__(tpb_strent_kernel*tpb_strent_kernel) void strent_kernel_linear(scalar_chains* chain_heads, float *tdt, int *d_offset, float4 *d_new_strent, float *d_new_tau_CD, float* d_new_cr_time)
{
    ...
    int tz = chain_heads[j].Z[0]; //Current chain size
    surf2Dwrite((int)(i<tz), s_arm_index, 4 * i, j);
    ...
    if (i < tz - 1) {
        ...
        if (QN2.w > 1.0f) { //N=1 mean that shift is not possible, also it will lead to dividing by zero error
            ...
            float f1 = (i == 0) ? 2.0f * QN.w + 0.5f : QN.w;
            float f2 = (i == tz-2) ? 2.0f * QN2.w - 0.5f : QN2.w;
...
//Calculates probabilities of entanglement shuffle, creation and destruction by CD on all strands
//Memory and kernel structure: 2D kernel; i- entanglement index j - chain index
template<int type> __global__ __launch_bounds__(tpb_strent_kernel*tpb_strent_kernel) void strent_kernel_star(scalar_chains* chain_heads, float *tdt, int *d_offset, float4 *d_new_strent, float *d_new_tau_CD, float* d_new_cr_time)
{
    //Calculate kernel index
    int i = blockIdx.x * blockDim.x + threadIdx.x;//strent index
    int j = blockIdx.y * blockDim.y + threadIdx.y;//chain index
...
__global__ void head_tail_kernel_linear(scalar_chains* chain_heads, int *d_offset, float4 *d_new_strent, float *d_new_tau_CD){
    ...
    float4 QNtailp; //previous before last strent
    ...
    int4 probs_a = make_int4(0,0,0,0); //variable for storing calculated probabilities at the head of the chain
    //x - destruction in the beginning by SD
    //y - creation    in the beginning by SD

    int4 probs_z = make_int4(0,0,0,0); //variable for storing calculated probabilities at the tail of the chain
    //x - destruction at the end    by SD
    //y - creation    at the end    by SD
    //w - creation                  by CD

    QNtail = fetch_new_strent(tz - 1, oft) ? new_strent : tex2D(t_a_QN, make_offset(tz - 1, oft), i);
    QNhead = fetch_new_strent(0, oft) ? new_strent : tex2D(t_a_QN, make_offset(0, oft), i);

    if (tz == 1) {
        probs_a.y = (int)(PROBS_CUTOFF*__fdividef(1.0f, (dBe * dnk_arms[0])));//Creation at the end by SD
        probs_z.y = (int)(PROBS_CUTOFF*__fdividef(1.0f, (dBe * dnk_arms[0])));//Creation at the end by SD
    } else {
        if (QNhead.w == 1.0f) {//destruction by SD in the beginning
            QNheadn = fetch_new_strent(tz - 2, oft) ? new_strent : tex2D(t_a_QN, make_offset(tz - 2, oft), i);
            float f2 = (tz == 2) ? QNheadn.w + 0.25f : 0.5f * QNheadn.w;
            probs_a.x = (int)(PROBS_CUTOFF*__fdividef(1.0f, f2 + 0.75f));
        } else {//creation by SD in the beginning
            probs_a.y = (int)(PROBS_CUTOFF*__fdividef(2.0f, dBe * (QNhead.w + 0.5f)));
        }
        if (QNtail.w == 1.0f) {//destruction by SD at the end
            QNtailp = fetch_new_strent(1, oft) ? new_strent : tex2D(t_a_QN, make_offset(1, oft), i);
    ...
    surf2Dwrite(probs_z, s_probs, sizeof(int4)*(tz - 1), i);
    surf2Dwrite(probs_a, s_probs, sizeof(int4)*(tz), i);
}

__global__ void tail_kernel_star(scalar_chains* chain_heads, int *d_offset, float4 *d_new_strent){
    ...
    int4 probs_z = make_int4(0,0,0,0); //variable for storing calculated probabilities at the tail of chain
    //x - destruction at the end    by SD
    //y - creation    at the end    by SD
    //w - creation    in the middle by CD

    QNtail = fetch_new_strent(tz - 1 + run_sum, oft) ? new_strent : tex2D(t_a_QN, make_offset((tz - 1)+ run_sum, oft), i);
    if (tz == 1) {
        probs_z.y = (int)(PROBS_CUTOFF*__fdividef(1.0f, (dBe * dnk_arms[arm])));//Creation at the end by SD
    } else {
        if (QNtail.w == 1.0f) {//destruction by SD at the end
            QNtailp = fetch_new_strent(tz - 2 + run_sum, oft) ? new_strent : tex2D(t_a_QN, make_offset(tz - 2 + run_sum, oft), i);
            float f1 = (tz == 2) ? QNtailp.w + 0.25f : 0.5f * QNtailp.w;
            probs_z.x = (int)(PROBS_CUTOFF*__fdividef(1.0f, f1 + 0.75f));
        } else {//creation by SD at the end
            probs_z.y = (int)(PROBS_CUTOFF*__fdividef(2.0f, dBe * (QNtail.w + 0.5f)));
...
template<int narms> __global__ void head_kernel_star(scalar_chains* chain_heads, int *d_offset, float4 *d_new_strent, float *d_new_tau_CD){
...
__global__ void scan_kernel(scalar_chains* chain_heads, int *rand_used, int* found_index,  int* found_shift, float* add_rand) {
	...
	//parallel scan in s
    //if (i<=chain_heads[j].Z[0]) printf("\n%i\t%i\t%i\t%i\t%i",i,temp.x, temp.y, temp.z, temp.w);
...
__device__ void apply_shuffle_linear(int k, int i, int j, float4 new_strent, scalar_chains* chain_heads, int *d_offset) {
    ...
    //if (type == 1) { //Apply flow if specified
    //    QN1 = kappa(QN1, olddt);
    //    QN2 = kappa(QN2, olddt);
    //}
...
__device__ void apply_shuffle_star(int k, int i, int j, float4 new_strent, scalar_chains* chain_heads, int *d_offset) {
    ...
    if (jj == 0) {//shuffling invoving branch-point
        ...
        int run_sum_ = 0;
        for (int arm_ = 0; arm_<d_narms; arm_++) {
            if ((chain_heads[i].Z[arm_]>1) && (arm_ != arm)) {//entangled arms
               temp_ = read_strent(i, 0 + run_sum_, d_offset[i*d_narms + arm_], new_strent);
                sumNinv += 1 / temp_.w;
            }
            run_sum_ += d_z_max_arms[arm_];
        }
        ...
        run_sum_ = 0;
        for (int arm_ = 0; arm_ < d_narms; arm_++) {
            if ((chain_heads[i].Z[arm_] > 1) && (arm_ != arm)) {//update Q_1^i if arm is entangled
                surf2Dwrite(read_strent(i, 0 + run_sum_, d_offset[i*d_narms + arm_], new_strent) - deltaQ, s_b_QN, 16 * run_sum_, i);
            }
            run_sum_ += d_z_max_arms[arm_];
        }
        QN1 = QN1 - deltaQ; //update Q_1 for current arm as well
    }
    surf2Dwrite(QN1, s_b_QN, 16 * j, i);
    surf2Dwrite(QN2, s_b_QN, 16 * (j + 1), i);
...
__device__ void apply_destroy_linear(int k, int i, int j, float4 new_strent, scalar_chains* chain_heads, int *d_offset, float* d_new_cr_time){
    //setup local variables
    int tz = chain_heads[i].Z[0];
    uint oft = d_offset[i];
...
__device__ void apply_destroy_star(int k, int i, int j, float4 new_strent, scalar_chains* chain_heads, int *d_offset, float* d_new_cr_time){
    int arm=0;
    int run_sum=0;
    for (arm=0; j>=run_sum+d_z_max_arms[arm]; arm++){
        run_sum+=d_z_max_arms[arm];
    ...
    if (chain_heads[i].Z[arm]==1) { //arm go unentangled as a result of entanglement destruction
        ...
        for (int arm_ = 0; arm_<d_narms; arm_++) {
            ...
            if ((tz_>1) && (arm_ != arm)) {//entangled arms
                unent = false;
                sumNinv += 1 / read_strent(i, 0 + run_sum_, d_offset[i*d_narms + arm_], new_strent).w;
            }
            run_sum_ += d_z_max_arms[arm_];
        }
...
__device__ void apply_create_SD_linear(int k, int i, int j, float4 new_strent, scalar_chains* chain_heads, int *d_offset, float* d_new_cr_time, int *tau_CD_used_SD, float4 *d_new_strent, float *d_new_tau_CD, float* add_rand){
    //setup local variables
    int tz = chain_heads[i].Z[0];
    uint oft = d_offset[i];

    float4 QN1 = read_strent(i, j, oft, new_strent); //plays role of the tail or head depending on the place in the chain (j should be either 0 or tz-1)

    //Apply shift
    float4 temp = tex2D(t_taucd_gauss_rand_SD, tau_CD_used_SD[i], i); //read random strand + random characteristic lifetime
    tau_CD_used_SD[i]++;
    chain_heads[i].Z[0]++;
    ...
    if (k==6){ //head
        ...
        surf2Dwrite(temp, s_b_QN, 16 * j, i); //TODO maybe deformation should be applied here
    }
    else if (k==3){ //tail
        d_new_strent[i] = temp;
...
__device__ void apply_create_SD_star(int k, int i, int j, float4 new_strent, scalar_chains* chain_heads, int *d_offset, float* d_new_cr_time, int *tau_CD_used_SD, float4 *d_new_strent, float *d_new_tau_CD, float* add_rand){
    int arm=0;
    int run_sum=0;
    for (arm=0; j>=run_sum+d_z_max_arms[arm]; arm++){
        run_sum+=d_z_max_arms[arm];
        ...
        float *d_new_tau_CD, float* d_new_cr_time, int *d_write_time, int correlator_type, int *rand_used, int *tau_CD_used_CD, int *tau_CD_used_SD, int* found_index, int* found_shift, float* add_rand)
{
    ...
    int j = found_index[i];  //strent coordinate
    int k = found_shift[i];  //type of jump
    ...
    //Use Kahan summation algorithm to update time of the chain
    ...
    rand_used[i]++;
    //printf("\nTime %f\tZ %i\tStrent %i\tJump %i",chain_heads[i].time, chain_heads[i].Z[0], j, k);
